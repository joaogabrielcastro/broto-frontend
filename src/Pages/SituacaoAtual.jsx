...
const SituacaoAtual = () => {
  ...
  const carregarViagens = () => {
    ...
    axios.get(`${API_BASE_URL}/situacao-atual-caminhoes`)
      .then(res => {
        ...
        console.log("Viagens carregadas:", validViagens);
  ...
  const handlePassarParaCustos = (viagemId) => {
    console.log("Tentando remover viagemId:", viagemId); 
    
    // Remove a viagem do estado local IMEDIATAMENTE
    setViagens(prevViagens => {
      ...
      console.log("Número de viagens após filtro:", filtered.length); 
      return filtered;
    });
